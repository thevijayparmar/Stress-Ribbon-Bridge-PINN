...
# =====================================================================
is_paused = False
stop_requested = False  # Kept so the loop doesn't crash, but can no longer be triggered
...
# Only listening for 'P' now. The 'S' listener has been completely removed.
...
train_files = sorted(glob.glob(os.path.join(BASE_DIR, '#01_Generator_0.84M_V*.csv')))
val_files = sorted(glob.glob(os.path.join(BASE_DIR, '#02_Generator_Validation_0.25M_V6*.csv')))
...
json_path = os.path.join(BASE_DIR, 'Translation_Dictionary_Long.json')
with open(json_path, 'w') as f:
    json.dump(scale_bounds, f, indent=4)
print(f"💾 SUCCESS: Saved 'Translation_Dictionary_Long.json' to {BASE_DIR}")
...
# --- REVISED PHYSICS LOSS FUNCTION ---
def physics_loss_fn(Y_pred_scaled, X_scaled):
    # 1. Extract real w and a
    w_real = X_scaled[:, 2] * w_range + w_min
    a_real = Y_pred_scaled[:, 0] * a_range + a_min
    
    # 2. Calculate what H SHOULD be based on exact physics
    H_physics_real = w_real * a_real
    
    # 3. CRITICAL FIX: Scale the physics H back to a 0.0 - 1.0 range
    H_physics_scaled = (H_physics_real - H_min) / H_range
    
    # 4. Compare it to the AI's predicted H (which is already scaled 0-1)
    H_pred_scaled = Y_pred_scaled[:, 1]
    
    # 5. Return the normalized penalty
    return torch.mean((H_pred_scaled - H_physics_scaled) ** 2)
# --------------------------------------

# Helper function for live system stats
...
for epoch in epoch_bar:
    ...
    for X_batch, Y_batch in batch_bar:
        ...
        # Update Live Hardware Stats on the Progress Bar
    ...
    if current_epoch >= threshold_epoch:
        if mean_phys_err < best_phys_error:
            ...
            # Delete old BEST files to save space
            ...
            # Export new BEST ONNX immediately
        ...
        # Before Epoch 400, just show the error without saving
...
# 3. NATIVE PYTORCH BACKUPS (Failsafe)
